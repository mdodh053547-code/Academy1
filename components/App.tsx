...
const App: React.FC = () => {
  ...
  const selectCoachProfile = (coach: any) => {
    ...
    if (!currentCoach?.password) {
      ...
      setPortal('staff');
      setActiveTab('dashboard'); // Start with Dashboard to see urgent tasks
  ...
  const filteredPlayersForLogin = playersList.filter(p => 
  ...
  const renderContent = () => {
    ...
    if (portal === 'member') {
      return <MemberPortal onBack={logout} player={loggedInMember} />;
    }
    ...
    if (portal === 'visitor') {
      return (
        <VisitorPortal 
          ...
          onRegister={() => setPortal('registration')} 
          tourData={tourData}
        />
      );
    }
    ...
    if (portal === 'registration') {
      return (
        <div className="min-h-screen bg-gray-50 p-6 md:p-20">
          <PlayerRegistration 
            onBack={() => setPortal('visitor')} 
            onSuccess={() => {
              setPortal('landing');
              alert("تم تقديم طلب انضمام بنجاح! سيتم التواصل معكم قريباً.");
            }} 
          />
        </div>
      );
    ...
    switch (activeTab) {
      case 'dashboard':
        return (
          <Dashboard 
            ...
            onRegister={() => setActiveTab('players')} 
      ...
      case 'players':
        return <PlayerList restrictedTeam={coachTeam} />;
      case 'teams':
        return <TeamsPanel restrictedTeam={coachTeam} />;
      case 'attendance':
        return <AttendancePanel restrictedTeam={coachTeam} />;
      case 'performance':
        return <PerformancePanel restrictedTeam={coachTeam} />;
      case 'finance':
        return <FinancePanel />;
      case 'communication':
      ...
      case 'settings':
        return (
          <SettingsPanel 
            coaches={coachesList} 
            onCoachesUpdate={setCoachesList} 
            adminProfile={adminProfile}
            onAdminUpdate={setAdminProfile}
            academyLinks={academyLinks}
            onAcademyLinksUpdate={setAcademyLinks}
            tourData={tourData}
            onTourDataUpdate={setTourData}
            players={playersList}
          />
        );
      default:
  ...
  const RoleSwitcher = (
    <div className="flex items-center gap-2 bg-gray-50 p-1.5 rounded-2xl border border-gray-100 shadow-inner">
      <div className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-xs font-bold transition-all ${
        userRole === UserRole.ADMIN 
        ? 'bg-emerald-600 text-white shadow-md' 
        : 'text-gray-500'
      }`}>
        <UserCog size={14} />
        {userRole === UserRole.ADMIN ? 'مدير النظام' : 'مدرب'}
      </div>
              ...
              <div className="flex justify-between items-center">
                <div className="w-12">
                  {selectedProfileForLogin && (
                    <button 
                      onClick={() => { setSelectedProfileForLogin(null); setLoginError(''); }} 
                      className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-full transition-all flex items-center gap-1"
                    >
                      <ArrowRight size={20} />
                    </button>
                  )}
                ...
                <div className="flex flex-col items-center">
                   <div className="p-3 bg-emerald-100 text-emerald-600 rounded-2xl mb-2">
                      {loginType === 'member' ? <Users size={24} /> : <ShieldCheck size={24} />}
                   </div>
                   <h3 className="text-2xl font-black text-gray-800 tracking-tighter">
                    {loginType === 'member' ? 'دخول بوابة الأعضاء' : 'دخول الكادر الإداري'}
                   </h3>
                </div>
              ...
              {!selectedProfileForLogin ? (
                <div className="space-y-6">
                  {loginType === 'staff' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-4">
                      <button 
                        onClick={handleAdminLoginInitiate} 
                        className="group relative bg-emerald-900 p-8 rounded-[2.5rem] text-white flex flex-col items-center gap-4 transition-all hover:scale-[1.02] shadow-xl border-2 border-transparent hover:border-emerald-400"
                      >
                        <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center text-emerald-950 shadow-inner">
                          <Crown size={40} />
                        </div>
                        <div className="text-center">
                          <h4 className="text-xl font-black">{adminProfile.name}</h4>
                          <p className="text-emerald-400 text-xs font-bold uppercase tracking-widest mt-1">المدير العام</p>
                        </div>
                      </button>
                      <div className="max-h-[350px] overflow-y-auto pr-2 custom-scrollbar space-y-3">
                        ...
                        {coachesList.map((coach) => (
                          <button 
                            key={coach.id} 
                            onClick={() => selectCoachProfile(coach)} 
                            className="w-full bg-gray-50 p-5 rounded-2xl border border-gray-100 flex items-center gap-4 transition-all hover:bg-white hover:border-emerald-500 hover:shadow-lg group"
                          >
                            <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center font-black text-lg group-hover:bg-emerald-600 group-hover:text-white transition-all">
                              {coach.name.charAt(7)}
                            </div>
                            <div className="text-right flex-1">
                              <p className="text-sm font-black text-gray-800">{coach.name}</p>
                              <p className="text-[10px] text-gray-500 font-bold uppercase">{coach.team}</p>
                            </div>
                    ...
                    <div className="space-y-8 py-4">
                      <div className="relative max-w-md mx-auto">
                        ...
                        <input 
                          type="text" 
                          placeholder="ابحث عن اسمك للدخول..." 
                          className="w-full pr-14 pl-6 py-5 bg-gray-50 border border-gray-200 rounded-[2rem] outline-none font-bold text-lg focus:ring-4 focus:ring-blue-500/10 focus:bg-white transition-all shadow-inner"
                          value={playerSearchQuery}
                          onChange={(e) => setPlayerSearchQuery(e.target.value)}
                        />
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-right">
                        {filteredPlayersForLogin.length > 0 ? filteredPlayersForLogin.map(player => (
                          <button 
                            key={player.id} 
                            onClick={() => { setSelectedProfileForLogin(player); setLoginError(''); }}
                            className="p-5 bg-blue-50 border border-blue-100 rounded-[2rem] flex items-center gap-4 hover:bg-blue-600 hover:text-white transition-all group shadow-sm"
                          >
                            <div className="w-12 h-12 bg-white text-blue-600 rounded-2xl flex items-center justify-center font-black text-xl group-hover:bg-blue-500 group-hover:text-white transition-all shadow-sm">
                              {player.fullName?.charAt(0)}
                            </div>
                            <div className="flex-1">
                              <p className="text-sm font-black leading-none mb-1.5">{player.fullName}</p>
                              <p className="text-[10px] opacity-70 font-bold uppercase tracking-tighter">{player.team}</p>
                            </div>
                            <ChevronLeft size={18} className="opacity-40 group-hover:opacity-100 transition-all" />
                          </button>
                        )) : (
                          <div className="col-span-full py-16 text-center text-gray-400 font-bold italic border-2 border-dashed border-gray-100 rounded-[3rem]">
                            لا يوجد نتائج حالياً.. ابحث بالاسم الأول
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="py-10 max-w-md mx-auto w-full animate-in slide-in-from-bottom-6 duration-400 text-right">
                  <div className="flex flex-col items-center gap-4 mb-10">
                    <div className="w-24 h-24 bg-blue-600 text-white rounded-[2.5rem] flex items-center justify-center text-4xl font-black shadow-2xl shadow-blue-100">
                      {selectedProfileForLogin.fullName?.charAt(0) || selectedProfileForLogin.name.charAt(7)}
                    </div>
                    <div className="text-center">
                      <p className="text-gray-400 text-sm font-bold uppercase tracking-widest mb-1">تسجيل دخول باسم</p>
                      <h4 className="text-2xl font-black text-gray-800">{selectedProfileForLogin.fullName || selectedProfileForLogin.name}</h4>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handlePasswordLogin} className="space-y-8">
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          type="password" 
                          placeholder={loginType === 'member' ? "أدخل كلمة المرور (1234)" : "أدخل كلمة المرور"}
                          autoFocus
                          className="w-full pr-14 pl-8 py-5 bg-gray-50 border border-gray-200 rounded-[2rem] outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-black text-center text-xl tracking-[0.5em]"
                          value={loginPassword}
                    ...
                    {loginError && (
                      <div className="flex items-center gap-2 justify-center text-red-500 text-sm font-black animate-shake bg-red-50 py-3 rounded-2xl border border-red-100">
                        <AlertCircle size={18} />
                        {loginError}
                      </div>
                    )}
                    <button 
                      type="submit" 
                      className={`w-full py-5 rounded-[2rem] font-black shadow-2xl transition-all flex items-center justify-center gap-3 active:scale-95 text-lg ${
                        loginType === 'member' 
                    ...
                    >
                      <LogIn size={24} />
                      دخول باسم {selectedProfileForLogin.fullName?.split(' ')[0] || selectedProfileForLogin.name.split(' ')[0]}
                    </button>
                  </form>
                </div>
              )}
